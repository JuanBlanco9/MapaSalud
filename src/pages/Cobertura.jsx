...
// ── PMO tratamiento con dificultad ──────────────────────────────

function PmoTratamientoItem({ t, dif, fund }) {
  const [showInfo, setShowInfo] = useState(false);
  return (
      ...
      <div className="flex items-start gap-2">
        ...
        <div className="flex-1">
          <div className="flex flex-wrap items-center gap-1.5">
            ...
            {dif && (
              <span
                className={`inline-flex items-center gap-1 text-xs font-medium px-1.5 py-0.5 rounded-full ${dificultadStyles[dif]}`}
              >
                {dif === "directo" && <Check className="w-3 h-3" />}
                {dif === "tramite" && <Clock className="w-3 h-3" />}
                {dif === "dificil" && <AlertTriangle className="w-3 h-3" />}
          ...
          {showInfo && fund && (
            <div className="mt-2 pl-1 border-l-2 border-azul-200">
              <p className="text-xs text-gris-600">{fund.porque}</p>
              {fund.datosLitigiosidad && (
                <p className="text-xs text-gris-500 mt-1">{fund.datosLitigiosidad}</p>
              )}
              {fund.fallos && fund.fallos.length > 0 && (
                <div className="mt-1">
                  <p className="text-xs font-semibold text-gris-600">
                    Fallos ({fund.fallos.length}):
                  </p>
                  {fund.fallos.map((f, i) => (
                    <a
                      key={i}
          ...
          <div className="bg-white border border-gris-200 rounded-xl p-5">
            ...
            <div className="space-y-2.5">
              {pmo.tratamientos.map((t) => {
                ...
                return (
                  <PmoTratamientoItem key={t.tipo} t={t} dif={dif} fund={fund} />
                );
              })}
            </div>
          </div>
